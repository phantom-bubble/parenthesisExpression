...
/**
 * @brief  Æ¥ÅäÀ¨ºÅ
 * @param  tmpstr ´ıÆ¥ÅäµÄ×Ö·û´®
 * @return bool   Æ¥Åä³É¹¦·µ»Øtrue£¬·ñÔò·µ»Øfalse
 */
...
int main() {
	int a(1);
	std::string test = "(1$ÎÒï¿½ï¿½-jab2(*ÌÌÌÌÌÌdhidoaozzzZzw)-;2(&iwh£©ÖĞÎÄÀ¨ºÅ£¨72))()";
...
//	std::cout << findDeepestParenthesisIndex("((1+2)*3)") << std::endl;
	...
	std::cout << ReplaceDeepestParenthesisByString("1+2*(3+4)", addtionCut(FindDeepestParenthesis("(1+2*(3+4))"))) << std::endl;
}